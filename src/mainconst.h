...
const char *message2 = "Данная программа создана, чтобы проверить ваши знания "
                       "английского языка";
const char *message3 = "Введите S для начала и E для выхода";
const char *message4 = "Текущий глагол - ";
const char *message5
        = "Введите три формы глагола через пробел (Inf PS PP соответственно)";
...
{
    return (!strcmp(inf, dic[i].first_f) && !strcmp(ps, dic[i].second_f)
            && !strcmp(pp, dic[i].third_f));
}
